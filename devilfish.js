...
function pack(config) {
    ...
    function addCommonModule(fileName) {
        var commonModules = config.common;
        var commonStr = "\n";
        commonModules.forEach(function (item) {
            commonStr += "require('" + item + "');";
        });
        commonStr += "\n";

        var content = getFileContent(fileName, config.data);
        var splitContent = content.split(/(^define\s?\(\s?function\s?\(.*?{)/);
        if (splitContent.length === 3) {
            splitContent[1] += commonStr + splitContent[2];
    ...
    function getCMDContent (startFileName) {
        ...
        var content = addCommonModule(startFileName)

        moduelList[startFileName] = resoveRequire(startFileName, basePath, content);
        dealModuleId();
        return mergeFileContent();

        function resoveRequire(fileName, relativePath, fileContent) {
            var rootPath = process.cwd();
            var content = fileContent || getFileContent(fileName, config.data);
